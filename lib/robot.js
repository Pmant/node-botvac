...
function Robot(name, serial, secret) {
    ...
    this._beehiveBaseUrl = 'https://beehive.neatocloud.com:4443/vendors/neato/robots/';
    this.name = name;
    this._serial = serial;
    this._secret = secret;
...
Robot.prototype.getMapBoundaries = function getMapBoundaries(mapId, callback) {
    doAction(this, 'getMapBoundaries', {mapId}, function (error, result){
        if (typeof callback === 'function') {
            if (error) {
                callback(error, result);
            } else if ( result && 'data' in result && 'boundaries' in result.data ) {
                callback(null, {mapId, boundaries: result.data.boundaries});
            } else {
                callback(result && 'message' in result ? result.message: 'failed', result);
            }
        }
...
Robot.prototype.setMapBoundaries = function setMapBoundaries(mapId, boundaries, callback) {
    doAction(this, 'setMapBoundaries', {mapId, boundaries}, function (error, result){
        if (typeof callback === 'function') {
            if (error) {
                callback(error, result);
            } else if ( result && 'data' in result && 'boundaries' in result.data ){
